...
function solution($A, $B, $C) {
    ...
    for($i = 0 ; $i < count($C) ; $i++) {
        for($K = 0; $K < $totalPlanks; $K++) {
            if($C[$i] >= $A[$K] && $C[$i] <= $B[$K]) {
                $nailedPlanks[] = $i.$K.$A[$K].$B[$K];
            }
        }

        if(count($nailedPlanks) == $totalPlanks) {
            return count(array_unique(array_values($nailedPlanks)));
        }
    }
 ...
 * Task score 35%
 */
function solution2($starts, $ends, $nails) {
    $totalPlanks = count($starts);

    $nailedPlanks = [];

    for($i = 0 ; $i < count($nails) ; $i++) {
        for($K = 0; $K < $totalPlanks; $K++) {
            if($nails[$i] >= $starts[$K] && $nails[$i] <= $ends[$K]) {
                $nailedPlanks[$starts[$K].$ends[$K]] = 1;
            }
        }
    ...
    // if not all planks were nailed return -1
    if(count($nailedPlanks) < $totalPlanks) {
        return -1;
    }
...
function solution3($A, $B, $C) {
    ...
    for($i = 0 ; $i < count($C) ; $i++) {
        for($K = 0; $K < $totalPlanks; $K++) {
            if($C[$i] >= $A[$K] && $C[$i] <= $B[$K]) {
                if(!isset($nailedPlanks[$K.$A[$K].$B[$K]])){
                    $nailedPlanks[$K.$A[$K].$B[$K]] = 1;
                    $countNailedPlanks++;
                    if($countNailedPlanks == $totalPlanks) {
                        return $i+1;
                    }
                }
            }
...
print solution3([1, 4, 5, 8], [4, 5, 9, 10], [4, 6, 7, 10, 2]) ;
//print solution([1,3], [2,5], [1,4,4,4, 4]) ;
